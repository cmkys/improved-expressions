...
- **Image (variant) renaming**: in the expanded panel, the pencil on an image renames it. Keeping the `label` / `label-N` pattern keeps it in the group; any other name moves it out into its own (or another) expression. If the name is already taken, the image slots into that group with the next free `-N` number instead. Renaming an image that lives in an expression folder moves the file into the main character folder (SillyTavern has no move endpoint, so renames are re-upload + delete).
- **Rename merging**: renaming an expression to a name that already exists merges its images into that expression as the next free `-N` variants (tags follow along; an existing tag on the target wins). Spaces in any rename automatically become underscores.
...
- **Collapsible characters**: click a character's header to collapse it down to a one-line summary (expression/group counts) and click again to expand. Header buttons keep working while collapsed, and you can still drop image files onto a collapsed character to upload.
...
- **Folder layout**: SillyTavern's sprites API resolves at most two path segments (`character/subfolder`); anything deeper is silently truncated server-side. Expression folders and groups are therefore stored as *companion* folders next to the character folder — `MyCard/Alice.jumping/` rather than `MyCard/Alice/jumping/`. If you registered groups or expression folders on a version before this fix, delete and recreate those registrations (they were reading the main folder by mistake; your image files are safe in the main folder and can be re-sorted). A character's own folder path must contain at most one `/`.
